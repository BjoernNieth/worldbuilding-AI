...
d <- read.csv("stories_long_En.csv")
...
analyze_outcome <- function(data, outcome) {
  ...
  # We plot our space variable once to see its distribution
  # We are using a beta distributed GLM so we want to see if this is the correct assumption here
  hist(data[[outcome]], prob = TRUE, breaks = 50,
...
# Run multiple model diagnostics 
dir.create("diagnostics", showWarnings = FALSE)
diag_summary <- list()
for (name in names(results)) {
  cat("\n=== Diagnostics for:", name, "===\n")
  m_full <- results[[name]]$model_full
  
  sim <- simulateResiduals(m_full, n = 1000)
  
  # save plots to disk so you can attach them to the appendix
  png(file.path("diagnostics", paste0(name, "_qq.png")), 
      width = 800, height = 700)
  plotQQunif(sim, main = paste("QQ:", name))
  dev.off()
  
  png(file.path("diagnostics", paste0(name, "_resid_pred.png")),
      width = 800, height = 700)
  plotResiduals(sim, quantreg = TRUE)
  mtext(name, outer = TRUE, line = -1.5)
  dev.off()
  
  png(file.path("diagnostics", paste0(name, "_resid_by_model.png")),
      width = 1000, height = 700)
  plotResiduals(sim, form = d$model)
  dev.off()
  
  png(file.path("diagnostics", paste0(name, "_resid_by_section.png")),
      width = 1000, height = 700)
...
diag_table <- do.call(rbind, diag_summary)
write.csv(diag_table, "diagnostics/diagnostic_summary.csv", row.names = FALSE)
print(diag_table)