...
class Narrative_Beginnings_Ger(Base_Dataset_Class):
    def __init__(self, data_path, cutoffdate, n_shot=0, is_test=False):
        super().__init__()
        self.dataset = pd.read_csv(os.path.join(data_path, "beginnings_de.csv"))
        print(f"Dataset consists of {len(self.dataset)} samples")
        self.dataset = self.dataset.set_index("filename")
        if is_test:
            self.dataset = self.dataset.iloc[:10]
        self.i = None
        self.name= "Narrative_Beginnings_Ger"
        self.n_shot = n_shot
